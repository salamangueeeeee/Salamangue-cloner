...
--  Visible to everyone on the server (FE) via CatalogOnApplyOutfit.
--  Features:
--   - Click-to-Clone (Interactive in-game Raycast)
--   - Live Mirror / Stalker Mode (Auto-sync in real-time)
--   - Restore original skin (My Initial Skin)
--   - Equip by Direct ID (Custom Asset Equipper)
--   - Accessory Inspector with quick Asset ID copying
--   - Quick modifiers (Headless, Korblox Right Leg) with Favorites persistence
--   - Rainbow Skin RGB Mode (FE Chroma Cycle visible to all)
--   - Advanced 3D Controls (Manual drag rotation, Mouse wheel zoom & 3D Emotes)
--   - Auto-Reapply on Respawn (Anti-Reset)
--   - Outfit Randomizer / Mashup (Server skin mixer)
--   - Built-in presets & Skin Export/Import system
--   - Player sorting (Distance, Roblox Friends, Alphabetical)
--   - Customizable accent themes & Keyboard shortcuts (RightControl / F4)
-- ============================================================================
...
-- ============================================================================
--  2. CONFIGURATION, THEMES & PROPORTIONS
-- ============================================================================
local ScaleLimits = {
	HeadScale = { 0.1, 1.0, 1.0, "Head" },
	WidthScale = { 0.1, 1.0, 1.0, "Width" },
	HeightScale = { 0.1, 1.2, 1.0, "Height" },
	DepthScale = { 0.1, 1.2, 1.0, "Depth" },
	BodyTypeScale = { 0.0, 1.0, 0.0, "Body Type" },
...
local CopyOptions = {
	...
	scales = true,
	accessories = { hair = true, hat = true, face = true, neck = true, shoulder = true, front = true, back = true, waist = true, shirt = true, pants = true, jacket = true, sweater = true, shorts = true, leftShoe = true, rightShoe = true, dressSkirt = true, eyebrow = true, eyelash = true },
}
...
local Themes = {
	["Minimal White"] = { Accent = Color3.fromRGB(245, 245, 250), TextDark = Color3.fromRGB(15, 15, 18) },
	["Neon Cyan"]     = { Accent = Color3.fromRGB(0, 225, 255),   TextDark = Color3.fromRGB(10, 15, 20) },
	["Cyber Violet"]  = { Accent = Color3.fromRGB(180, 100, 255), TextDark = Color3.fromRGB(15, 10, 25) },
	["Crimson Red"]   = { Accent = Color3.fromRGB(255, 75, 90),   TextDark = Color3.fromRGB(20, 10, 12) },
	["Emerald Green"] = { Accent = Color3.fromRGB(80, 240, 140),  TextDark = Color3.fromRGB(10, 20, 15) },
}
local CurrentThemeName = "Minimal White"
...
-- ============================================================================
--  3.1 SERVER SNIPER & PRESENCE (JOIN A PLAYER BY USERNAME / USERID)
-- ============================================================================
...
local function sniperJoinPlayer(targetInput, statusCallback)
	...
	if targetInput == "" then
		updateStatus("Please enter a Username or UserID", false)
		return
	end

	task.spawn(function()
		updateStatus("Resolving player...", true)
		...
		if not userId or userId <= 0 then
			...
			if ok and uid and uid > 0 then
				...
				updateStatus("Roblox username not found", false)
				return
			end
		else
		...
		updateStatus("Checking status of @" .. userName .. "...", true)

		-- 1. Presence & direct JobId
		local presence = getPlayerPresence(userId)
		local targetPlaceId = game.PlaceId
		local gameTitle = (game.PlaceId == 4924922222 and "Brookhaven RP" or "this game")

		if presence then
			if presence.type == 0 then
				updateStatus("❌ @" .. userName .. " appears Offline (Searching...)", true)
				task.wait(0.5)
			elseif presence.type == 1 then
				updateStatus("🌐 @" .. userName .. " is on the Roblox website", true)
				task.wait(0.5)
			elseif presence.type == 2 then
				gameTitle = presence.gameName or "Detected game"
				if presence.placeId and presence.placeId > 0 then
					targetPlaceId = presence.placeId
				end
				updateStatus("🎮 @" .. userName .. " is in: " .. gameTitle, true)
				task.wait(0.5)

				if presence.gameId and type(presence.gameId) == "string" and #presence.gameId > 10 then
					updateStatus("🚀 Direct JobID found (" .. string.sub(presence.gameId, 1, 8) .. "...). Teleporting...", true)
					task.wait(0.5)
			...
			elseif presence.type == 3 then
				updateStatus("🛠️ @" .. userName .. " is in Roblox Studio", false)
				task.wait(0.5)
			end
		end

		updateStatus("Loading avatar of @" .. userName .. "...", true)
		...
		if not targetImgUrl or targetImgUrl == "" then
			updateStatus("Could not get player's avatar", false)
			return
		end
		...
		updateStatus("Scanning servers of " .. gameTitle .. "...", true)
		...
		while true do
			...
			updateStatus("Scan: " .. tostring(totalServersScanned) .. " servers checked...", true)
		...
		if foundServerId then
			updateStatus("Server found (" .. string.sub(foundServerId, 1, 8) .. "...). Teleporting to " .. gameTitle .. "...", true)
			task.wait(0.6)
			local okTp = pcall(function()
				TeleportService:TeleportToPlaceInstance(targetPlaceId, foundServerId, LocalPlayer)
			end)
			if not okTp then
				updateStatus("Teleport failed", false)
			end
		else
			updateStatus("Server not found (" .. tostring(totalServersScanned) .. " servers checked)", false)
		end
	end)
end

-- ============================================================================
--  4. SERIALIZATION & FAVORITES
-- ============================================================================
...
-- ============================================================================
--  5. BUILT-IN PRESETS CATALOG
-- ============================================================================
local BuiltInPresets = {
	{
		name = "Headless Slender",
		desc = "Modern black & white headless look",
		outfit = {
	...
	},
	{
		...
		desc = "Korblox Deathspeaker right leg",
		outfit = {
	...
	},
	{
		...
		desc = "Classic yellow, blue and green colors",
		outfit = {
	...
	{
		name = "Bacon Lord",
		desc = "The iconic legendary Pal Hair look",
		outfit = {
			Head = 0, Torso = 0, LeftArm = 0, RightArm = 0, LeftLeg = 0, RightLeg = 0,
	...
	},
	{
		...
		desc = "Futuristic katana and neon hood style",
		outfit = {
...
-- ============================================================================
--  6. HUMANOIDDESCRIPTION EXTRACTION & MANAGEMENT
-- ============================================================================
...
local function buildDescriptionTable(humdes, options, bodyScaleOverrides)
	...
	pcall(function() accTypeToSubKey[Enum.AccessoryType.Shoulder] = "shoulder" end)

	-- 1. Initialize outfit with player's current equipment (base)
	if base then
		local allProps = {
		...
		local allAccProps = {
			"FaceAccessory", "HatAccessory", "HairAccessory", "NeckAccessory", "ShoulderAccessory",
	...
	-- 2. Selectively replace ONLY checked categories from target
	if humdes then
		...
		-- Accessories & Hair: Preservation and Granular Replacement
		local finalAccs = {}
		local myAccs = {}
		if base then
		...
		-- (A) Preserve local player's accessories for all unchecked categories
		...
		-- (B) Add target's accessories for all checked categories
		...
		-- Accessory strings (Legacy)
		local accPropMap = {
			hair = "HairAccessory", hat = "HatAccessory", face = "FaceAccessory",
			neck = "NeckAccessory", shoulder = "ShoulderAccessory", front = "FrontAccessory", back = "BackAccessory",
...
local function DispatchDescriptionToServer(humdes, options, bodyScaleOverrides)
	if not ApplyOutfitRemote then
		return false, "Remote 'CatalogOnApplyOutfit' not found"
	end

	-- Capture initial state BEFORE any character modification
	...
	task.delay(1.5, function()
		if options.clothes and options.clothes.graphicTShirt and humdes and humdes.GraphicTShirt and humdes.GraphicTShirt ~= 0 then
			pcall(function() applyProperty("GraphicTShirt", humdes.GraphicTShirt) end)
		end
		if options.clothes and options.clothes.shirt and humdes and humdes.Shirt and humdes.Shirt ~= 0 then
			pcall(function() applyProperty("Shirt", humdes.Shirt) end)
		end
		if options.clothes and options.clothes.pants and humdes and humdes.Pants and humdes.Pants ~= 0 then
			pcall(function() applyProperty("Pants", humdes.Pants) end)
		end
	end)
	...
	task.delay(2.0, function()
		pcall(function()
			...
			if char then
				...
				if hum and CurrentHumDes then
					hum:ApplyDescription(CurrentHumDes)
				end
			end
		end)
	end)
...
StatusText.Text = "Ready (Press [RightControl] or [F4] to toggle)"
...
local tabDefs = {
	{ id = "Joueurs", label = "Players" },
	{ id = "HorsLigne", label = "Offline" },
	{ id = "Apercu", label = "Preview" },
	{ id = "Pieces", label = "Pieces" },
	{ id = "Taille", label = "Size" },
	{ id = "Favoris", label = "Favorites" },
...
-- ============================================================================
--  10. GLOBAL SKIN STATE VARIABLES & DISPLAY FUNCTIONS
-- ============================================================================
...
local function selectTargetAvatar(targetPlayer, targetUserId, displayName)
	...
	setStatus("Loading " .. SelectedTargetName .. "...", true)
	...
	task.spawn(function()
		...
		setStatus(SelectedTargetName .. " selected", true)
	end)
end

-- ============================================================================
--  11. MODULAR TAB CONSTRUCTION
-- ============================================================================
...
local function buildJoueursTab()
	...
	SearchInput.PlaceholderText = "Search for a player..."
	...
	local ClickToCloneBtn = createButton(SearchBarRow, "3D Click", C_CARD, CurrentTheme.Accent, UDim2.new(0, 110, 0, 26), UDim2.new(1, -116, 0.5, -13))
	...
	local sortDefs = { { id = "Distance", label = "Proximity" }, { id = "Amis", label = "Friends" }, { id = "Alphabetique", label = "A-Z" } }
	local sortBtns = {}
	...
	ClickToCloneBtn.MouseButton1Click:Connect(function()
		...
		if isClickToCloneActive then
			...
			setStatus("Click on a player in the game...", true)
...
local function buildHorsLigneTab()
	...
	OffTitle.Text = "Offline loading (Roblox API)"
	...
	OffBox.PlaceholderText = "Username or UserID..."
	...
	local OffSearchBtn = createButton(OffInputRow, "Load", CurrentTheme.Accent, CurrentTheme.TextDark, UDim2.new(0, 72, 0, 26), UDim2.new(1, -76, 0.5, -13))
	...
	OffResultPresence.Text = "Status: Loading..."

	local OffApplyBtn = createButton(OffResultCard, "Copy Skin", CurrentTheme.Accent, CurrentTheme.TextDark, UDim2.new(0.48, -4, 0, 28), UDim2.new(0, 12, 0, 88))
	local OffPreviewBtn = createButton(OffResultCard, "3D Preview", C_CARD, CurrentTheme.Accent, UDim2.new(0.48, -4, 0, 28), UDim2.new(0.52, 0, 0, 88))
	local OffJoinBtn = createButton(OffResultCard, "Join Their Server (Sniper)", C_CARD, CurrentTheme.Accent, UDim2.new(1, -24, 0, 32), UDim2.new(0, 12, 0, 124))
	...
	local function searchOffline(text)
		...
		task.spawn(function()
			...
			if not rId or rId <= 0 then
				...
				if ok and uid and uid > 0 then rId = uid else setStatus("Player not found", false) return end
			else
			...
			OffResultPresence.Text = "Status: Checking..."
			...
			if pres then
				...
				OffResultPresence.Text = "Status: Unavailable"
...
local function buildApercuTab()
	...
	PreviewTargetLabel.Text = "No avatar"
	...
	InspectorTitle.Text = "Parts & Asset IDs Inspector"
	...
	populateInspectorList = function(humdes)
		...
		local items = {}
		if humdes.Shirt and humdes.Shirt ~= 0 then table.insert(items, { type = "2D Shirt", id = humdes.Shirt }) end
		if humdes.Pants and humdes.Pants ~= 0 then table.insert(items, { type = "2D Pants", id = humdes.Pants }) end
		if humdes.Face and humdes.Face ~= 0 then table.insert(items, { type = "Face", id = humdes.Face }) end
		if humdes.Head and humdes.Head ~= 0 then table.insert(items, { type = "Head Mesh", id = humdes.Head }) end
		if humdes.RightLeg and humdes.RightLeg ~= 0 then table.insert(items, { type = "Right Leg", id = humdes.RightLeg }) end

		local rawAccs = {}
		pcall(function() rawAccs = humdes:GetAccessories(true) end)
		for _, a in ipairs(rawAccs) do
			if a.AssetId and a.AssetId ~= 0 then
				local tName = (typeof(a.AccessoryType) == "EnumItem" and a.AccessoryType.Name) or "Accessory"
		...
		if #items == 0 then
			...
			noItem.Text = "No accessories detected"
		...
		for _, it in ipairs(items) do
			...
			local copyIdBtn = createButton(row, "Copy ID", C_CARD, CurrentTheme.Accent, UDim2.new(0, 80, 0, 22), UDim2.new(1, -86, 0.5, -11))
			copyIdBtn.TextSize = 10
			copyIdBtn.MouseButton1Click:Connect(function()
				local ok = copyToClipboard(tostring(it.id))
				setStatus(ok and ("ID " .. tostring(it.id) .. " copied!") or "Clipboard unavailable", ok)
			end)
		end
	end
...
-- TAB 4: PIECES & FILTERS
local function buildPiecesTab()
	local PiecesTab = TabContainers["Pieces"]
	local pieceDefs = {
		{
			key = "body", label = "Body", desc = "Head, torso and limbs",
			subs = { { key = "head", label = "Head" }, { key = "torso", label = "Torso" }, { key = "leftArm", label = "Left Arm" }, { key = "rightArm", label = "Right Arm" }, { key = "leftLeg", label = "Left Leg" }, { key = "rightLeg", label = "Right Leg" } },
		},
		{ key = "face", label = "Face & Makeup", desc = "Face decal", subs = { { key = "decal", label = "Face decal" } } },
		{ key = "clothes", label = "2D Clothing", desc = "Classic shirts and pants", subs = { { key = "shirt", label = "Shirt" }, { key = "pants", label = "Pants" }, { key = "graphicTShirt", label = "Graphic T-Shirt" } } },
		{
			key = "accessories", label = "Accessories", desc = "Hair, hats, glasses, 3D clothing",
			subs = { { key = "hair", label = "Hair" }, { key = "hat", label = "Hats" }, { key = "face", label = "Glasses / Face" }, { key = "neck", label = "Neck" }, { key = "shoulder", label = "Shoulder" }, { key = "front", label = "Front" }, { key = "back", label = "Back" }, { key = "waist", label = "Waist" }, { key = "shirt", label = "3D Upper" }, { key = "pants", label = "3D Lower" }, { key = "jacket", label = "Jacket" }, { key = "sweater", label = "Sweater" }, { key = "shorts", label = "Shorts" }, { key = "leftShoe", label = "Left Shoe" }, { key = "rightShoe", label = "Right Shoe" }, { key = "dressSkirt", label = "Dress / Skirt" }, { key = "eyebrow", label = "Eyebrows" }, { key = "eyelash", label = "Eyelashes" } },
		},
		{
			key = "colors", label = "Skin Colors", desc = "Tones for 6 body parts",
			subs = { { key = "head", label = "Head" }, { key = "torso", label = "Torso" }, { key = "leftArm", label = "Left Arm" }, { key = "rightArm", label = "Right Arm" }, { key = "leftLeg", label = "Left Leg" }, { key = "rightLeg", label = "Right Leg" } },
		},
		{
			key = "animations", label = "Animations", desc = "Movement animation pack",
			subs = { { key = "idle", label = "Idle" }, { key = "walk", label = "Walk" }, { key = "run", label = "Run" }, { key = "jump", label = "Jump" }, { key = "fall", label = "Fall" }, { key = "climb", label = "Climb" }, { key = "swim", label = "Swim" }, { key = "mood", label = "Mood" } },
		},
		{ key = "scales", label = "Proportions", desc = "Size and body type", subs = {} },
	}
...
local function buildTailleTab()
	...
	local ApplyScalesBtn = createButton(TailleTab, "Apply proportions", CurrentTheme.Accent, CurrentTheme.TextDark, UDim2.new(1, 0, 0, 32))
	...
	local ResetScalesBtn = createButton(TailleTab, "Reset to target proportions", C_CARD, C_MUTED, UDim2.new(1, 0, 0, 30))
	ResetScalesBtn.MouseButton1Click:Connect(function()
		if BaseTargetScales then
			...
			setStatus("Target proportions restored", true)
		end
	end)
end

-- TAB 6: FAVORITES & PRESETS
local function buildFavorisTab()
	local FavorisTab = TabContainers["Favoris"]
	local SaveCurrentBtn = createButton(FavorisTab, "Save current skin", CurrentTheme.Accent, CurrentTheme.TextDark, UDim2.new(1, 0, 0, 32))
	...
	ImportBox.PlaceholderText = "Paste a skin code..."
	...
	local ImportBtn = createButton(CodeShareCard, "Import", C_CARD, CurrentTheme.Accent, UDim2.new(0, 80, 1, -10), UDim2.new(1, -85, 0, 5))
	...
	local function applyOutfitData(outfitPayload, skinName)
		...
		if not ready then setStatus("Wait " .. tostring(waitS) .. "s", false) return end
		setStatus("Applying " .. skinName .. "...", true)
		task.spawn(function()
			...
			-- 2. Update CurrentHumDes for 3D preview and future saves
			...
			-- 4. Apply proportions details and each body part individually
			...
			setStatus(skinName .. " applied with exact proportions!", ok)
		end)
	end
	...
	local function refreshSavedSkinsList()
		...
		if #SavedSkins == 0 then
			...
			emptyLabel.Text = "No favorites saved"
		...
		for idx, sk in ipairs(SavedSkins) do
			...
			nameEdit.PlaceholderText = "Favorite name"
			...
			local viewFavBtn = createButton(row, "View", C_CARD, CurrentTheme.Accent, UDim2.new(0, 40, 0, 26), UDim2.new(1, -146, 0.5, -13))
			local applyFavBtn = createButton(row, "Copy", CurrentTheme.Accent, CurrentTheme.TextDark, UDim2.new(0, 60, 0, 26), UDim2.new(1, -102, 0.5, -13))
			...
			exportBtn.MouseButton1Click:Connect(function()
				...
				if ok and encoded then
					...
					setStatus(cp and "Skin code copied!" or "Error", cp)
				end
			end)
			...
			viewFavBtn.MouseButton1Click:Connect(function()
				task.spawn(function()
					...
					SelectedTargetName = sk.name or "Favorite"
					refreshAll3DPreviews(targetDes, sk.name)
					populateInspectorList(targetDes)
					switchTab("Apercu")
					setStatus(sk.name .. " loaded with exact proportions", true)
				end)
			end)
			...
			delFavBtn.MouseButton1Click:Connect(function()
				...
				setStatus("Favorite deleted", false)
			end)
		end
	end
	...
	SaveCurrentBtn.MouseButton1Click:Connect(function()
		...
		persistSkinsToFile()
		refreshSavedSkinsList()
		setStatus("Skin and exact proportions saved!", true)
	end)
	...
	ImportBtn.MouseButton1Click:Connect(function()
		...
		if ok and type(decoded) == "table" then
			table.insert(SavedSkins, { name = "Imported " .. (#SavedSkins + 1), date = os.date("%d/%m %H:%M"), outfit = deserializeValue(decoded) })
			persistSkinsToFile()
			refreshSavedSkinsList()
			ImportBox.Text = ""
			setStatus("Skin imported successfully!", true)
		else
	...
	for _, pr in ipairs(BuiltInPresets) do
		...
		local prApplyBtn = createButton(row, "Apply", CurrentTheme.Accent, CurrentTheme.TextDark, UDim2.new(0, 68, 0, 26), UDim2.new(1, -74, 0.5, -13))
...
local function buildOutilsTab()
	...
	CustomEquipTitle.Text = "Equip a Roblox Asset by ID"
	...
	CustomIdInput.PlaceholderText = "Enter a Roblox Asset ID (e.g. 139607718)..."
	...
	local customShoulderEnum = nil
	pcall(function() customShoulderEnum = Enum.AccessoryType.Shoulder end)

	local customCategories = {
		{ id = "Hat", label = "Hat", enum = Enum.AccessoryType.Hat },
		{ id = "Hair", label = "Hair", enum = Enum.AccessoryType.Hair },
		{ id = "Face", label = "Glasses", enum = Enum.AccessoryType.Face },
		{ id = "Shoulder", label = "Shoulder", enum = customShoulderEnum },
		{ id = "Shirt", label = "Shirt", prop = "Shirt" },
		{ id = "Pants", label = "Pants", prop = "Pants" },
		{ id = "RightLeg", label = "R. Leg", prop = "RightLeg" },
		{ id = "Head", label = "Head", prop = "Head" },
	}
	...
	local CustomEquipActionBtn = createButton(CustomEquipCard, "Equip this skin by id", CurrentTheme.Accent, CurrentTheme.TextDark, UDim2.new(1, -20, 0, 24), UDim2.new(0, 10, 0, 86))
	...
	CustomEquipActionBtn.MouseButton1Click:Connect(function()
		...
		if not idNum or idNum <= 0 then setStatus("Please enter a valid Asset ID", false) return end
			...
			for _, cDef in ipairs(customCategories) do
				if cDef.id == customCategorySelected then
					...
					elseif cDef.enum then
						...
						setStatus("This accessory type is not available on your Roblox version", false)
						return
					end
				end
			end
		end)
		if not ok then setStatus("Error: " .. tostring(err), false) return end
		refreshAll3DPreviews()
		populateInspectorList(CurrentHumDes)
		setStatus("Asset ID " .. tostring(idNum) .. " equipped!", true)
	end)
	...
	RainbowTitle.Text = "Rainbow Skin Mode (FE RGB Chroma)"
	...
	local RainbowToggleBtn = createButton(RainbowCard, "Enable Rainbow Skin Mode", C_CARD, CurrentTheme.Accent, UDim2.new(1, -20, 0, 32), UDim2.new(0, 10, 0, 34))
	...
	RainbowToggleBtn.MouseButton1Click:Connect(function()
		...
		if isRainbowActive then
			...
			RainbowToggleBtn.Text = "Disable Rainbow Mode"
			setStatus("Rainbow Skin Mode enabled!", true)
			...
			RainbowToggleBtn.Text = "Enable Rainbow Skin Mode"
			if rainbowThread then pcall(function() task.cancel(rainbowThread) end) end
			setStatus("Rainbow Mode disabled", false)
		end
	end)

	-- 3. Quick Modifiers (Headless, Korblox, Shoulder)
	...
	ModifTitle.Text = "Quick Modifiers (Saved in Favorites)"
	...
	local HeadlessBtn = createButton(ModifCard, "Headless", C_CARD, CurrentTheme.Accent, UDim2.new(0.31, -4, 0, 32), UDim2.new(0, 10, 0, 34))
	local KorbloxBtn = createButton(ModifCard, "Korblox Leg", C_CARD, CurrentTheme.Accent, UDim2.new(0.31, -4, 0, 32), UDim2.new(0.34, 0, 0, 34))
	local ShoulderBtn = createButton(ModifCard, "Shoulder", C_CARD, CurrentTheme.Accent, UDim2.new(0.31, -4, 0, 32), UDim2.new(0.68, 0, 0, 34))
	...
	HeadlessBtn.MouseButton1Click:Connect(function()
		...
		setStatus("Headless applied and injected into skin!", true)
	end)
	...
	KorbloxBtn.MouseButton1Click:Connect(function()
		...
		setStatus("Korblox leg applied and injected into skin!", true)
	end)
	...
	ShoulderBtn.MouseButton1Click:Connect(function()
		...
		if CurrentHumDes then
			local currentAccs = {}
			pcall(function() currentAccs = CurrentHumDes:GetAccessories(true) end)
			table.insert(currentAccs, { AssetId = 150381051, AccessoryType = Enum.AccessoryType.Shoulder, Order = #currentAccs + 1 })
			pcall(function() CurrentHumDes:SetAccessories(currentAccs, true) end)
			refreshAll3DPreviews()
			populateInspectorList(CurrentHumDes)
		end
		pcall(function() applyAccessory(Enum.AccessoryType.Shoulder, 150381051, 1) end)
		setStatus("Shoulder applied and injected into skin!", true)
	end)
	...
	RespawnTitle.Text = "Auto-Reapply on Respawn"
	...
	RespawnSub.Text = "Automatically re-applies your active skin after each death/reset."
	...
	-- 5. Restore original skin
	local RestoreOriginalBtn = createButton(OutilsTab, "Restore my original skin", CurrentTheme.Accent, CurrentTheme.TextDark, UDim2.new(1, 0, 0, 34))
	RestoreOriginalBtn.MouseButton1Click:Connect(function()
		if OriginalPlayerDescription then
			...
			SelectedTargetName = "My Initial Skin"
			_G.SalamangueClonerApply()
			refreshAll3DPreviews()
			setStatus("Original skin restored!", true)
		end
	end)
	...
	local RandBtn = createButton(RandomizerCard, "Generate Random Hybrid Skin", CurrentTheme.Accent, CurrentTheme.TextDark, UDim2.new(1, -20, 0, 32), UDim2.new(0, 10, 0, 24))
	RandBtn.MouseButton1Click:Connect(function()
		local plrs = Players:GetPlayers()
		if #plrs <= 1 then setStatus("Not enough players", false) return end
		...
		task.spawn(function()
			...
			SelectedTargetName = "Hybrid Mashup"
			_G.SalamangueClonerApply()
			refreshAll3DPreviews()
			setStatus("Hybrid skin generated!", true)
		end)
	end)
	...
	local StalkerToggleBtn = createButton(StalkerCard, "Enable Real-Time Tracking", C_CARD, CurrentTheme.Accent, UDim2.new(1, -20, 0, 32), UDim2.new(0, 10, 0, 24))
	...
	StalkerToggleBtn.MouseButton1Click:Connect(function()
		...
		if isStalkerActive then
			if not SelectedTargetPlayer then
				...
				setStatus("Select a player first", false)
				return
			end
			StalkerToggleBtn.BackgroundColor3 = CurrentTheme.Accent
			StalkerToggleBtn.TextColor3 = CurrentTheme.TextDark
			StalkerToggleBtn.Text = "Disable Tracking"
			setStatus("Tracking enabled on @" .. SelectedTargetPlayer.Name, true)
			...
			StalkerToggleBtn.Text = "Enable Real-Time Tracking"
			if stalkerThread then pcall(function() task.cancel(stalkerThread) end) end
			setStatus("Tracking disabled", false)
		end
	end)

	-- 8. Server Sniper (Join a Player)
	...
	SniperTitle.Text = "Join a Player (Server Sniper)"
	...
	SniperInput.PlaceholderText = "Enter a Username or UserID to join..."
	...
	local SniperActionBtn = createButton(SniperCard, "Find & Join Their Server", CurrentTheme.Accent, CurrentTheme.TextDark, UDim2.new(1, -20, 0, 28), UDim2.new(0, 10, 0, 58))
	...
	SniperStatusLbl.Text = "Enter a username to detect their game and server"
	...
	local tNames = { "Minimal White", "Neon Cyan", "Cyber Violet", "Crimson Red", "Emerald Green" }
	for idx, thName in ipairs(tNames) do
		...
		tBtn.MouseButton1Click:Connect(function()
			...
			CurrentTheme = th
			persistSettings()
			setStatus("Theme " .. thName .. " enabled! Restart recommended.", true)
		end)
	end
end

-- ============================================================================
--  12. BOTTOM BAR & MAIN CONTROL
-- ============================================================================
...
local MainCopyBtn = createButton(BottomBar, "COPY SKIN", CurrentTheme.Accent, CurrentTheme.TextDark, UDim2.new(0.72, -4, 1, -6), UDim2.new(0, 3, 0, 3))
MainCopyBtn.Font = Enum.Font.GothamBold
MainCopyBtn.TextSize = 12

local MainUnloadBtn = createButton(BottomBar, "Close", C_CARD, C_MUTED, UDim2.new(0.28, -4, 1, -6), UDim2.new(0.72, 4, 0, 3))

local function ApplyCurrentSelectedSkin()
	if not CurrentHumDes then setStatus("Select a player first", false) return false end
	local ready, waitS = canApplyOutfit()
	if not ready then setStatus("Wait " .. tostring(waitS) .. "s", false) return false end
	local targetName = SelectedTargetName or "Target"
	setStatus("Copying " .. targetName .. "...", true)
	task.spawn(function()
		local ok = DispatchDescriptionToServer(CurrentHumDes, CopyOptions, ScaleOptions)
		if ok then
			setStatus(targetName .. " copied successfully", true)
			if isOptionActive(CopyOptions.animations) then forceAnimationOverride(CurrentHumDes, nil) end
		else
			setStatus("Error during copy", false)
		end
	end)
	return true
end
...
local function buildInventoryTab()
	local InvTab = TabContainers["Inventory"]

	for _, ch in pairs(InvTab:GetChildren()) do
		if not ch:IsA("UIListLayout") and not ch:IsA("UIPadding") then
			ch:Destroy()
		end
	end

	local RefreshAllBtn = createButton(InvTab, "Refresh Inventory", CurrentTheme.Accent, CurrentTheme.TextDark, UDim2.new(1, 0, 0, 30))

	local InvScroll = Instance.new("ScrollingFrame")
	InvScroll.Parent = InvTab
	InvScroll.Position = UDim2.new(0, 0, 0, 36)
	InvScroll.Size = UDim2.new(1, 0, 1, -36)
	InvScroll.BackgroundTransparency = 1
	InvScroll.ScrollBarThickness = 4
	InvScroll.ScrollBarImageColor3 = CurrentTheme.Accent
	...
	local function makeInvHeader(parent, text, order)
	...
	local function rebuildOutfitFromHumDes()
		if not CurrentHumDes then return end
		local outfit = {}
		local props = { "Head", "Torso", "LeftArm", "RightArm", "LeftLeg", "RightLeg", "Shirt", "Pants", "Face", "GraphicTShirt", "HeadColor", "TorsoColor", "LeftArmColor", "RightArmColor", "LeftLegColor", "RightLegColor" }
		for _, prop in ipairs(props) do
			local val = 0
			pcall(function() val = CurrentHumDes[prop] end)
			if val and val ~= 0 then outfit[prop] = val end
		end
		local scaleProps = { "HeadScale", "WidthScale", "HeightScale", "DepthScale", "BodyTypeScale", "ProportionScale" }
		for _, sp in ipairs(scaleProps) do
			pcall(function()
				if CurrentHumDes[sp] ~= nil then outfit[sp] = CurrentHumDes[sp] end
			end)
		end
		local accs = {}
		pcall(function() accs = CurrentHumDes:GetAccessories(true) end)
		if #accs > 0 then outfit.Accessories = accs end
		return outfit
	end

	local function buildInvCard(assetId, name, subtext, category, layoutOrder, removeFn)
		...
		preview.Parent = card
		preview.Position = UDim2.new(0, 8, 0.5, -22)
		preview.Size = UDim2.new(0, 44, 0, 44)
		preview.BackgroundColor3 = C_PANEL
		preview.BackgroundTransparency = 0.3
		preview.ScaleType = Enum.ScaleType.Fit
		preview.BorderSizePixel = 0
		preview.Image = "rbxthumb://type=Asset&id=" .. tostring(assetId) .. "&w=150&h=150"
		addCorner(preview, 6)
		...
		subLabel.Position = UDim2.new(0, 60, 0, 24)
		...
		catLabel.Position = UDim2.new(0, 60, 0, 40)
		catLabel.Size = UDim2.new(1, -120, 0, 12)
		catLabel.BackgroundTransparency = 1
		catLabel.Text = category
		catLabel.Font = Enum.Font.Gotham
		catLabel.TextSize = 8
		catLabel.TextColor3 = C_BORDER
		catLabel.TextXAlignment = Enum.TextXAlignment.Left

		local removeBtn = Instance.new("TextButton")
		removeBtn.Parent = card
		removeBtn.Position = UDim2.new(1, -38, 0.5, -12)
		...
		removeBtn.MouseButton1Click:Connect(function()
			if inventoryBusy then return end
			inventoryBusy = true
			removeBtn.Text = "..."
			removeBtn.BackgroundColor3 = Color3.fromRGB(100, 100, 100)

			local ok, err = pcall(function() removeFn() end)
			if not ok then
				setStatus("Remove failed: " .. tostring(err), false)
				removeBtn.Text = "X"
				removeBtn.BackgroundColor3 = Color3.fromRGB(180, 50, 50)
				inventoryBusy = false
				return
			...
			local outfit = rebuildOutfitFromHumDes()
			if outfit and ApplyOutfitRemote then
				pcall(function() ApplyOutfitRemote:FireServer(outfit) end)
				task.delay(0.35, function()
					pcall(function() ApplyOutfitRemote:FireServer(outfit) end)
				end)
	...
	local function refreshInventory()
		for _, ch in pairs(InvScroll:GetChildren()) do
							...
							if handle then
								...
								for _, v in ipairs(handle:GetDescendants()) do
									if v:IsA("StringValue") or v:IsA("IntValue") or v:IsA("NumberValue") then
										...
										if n and n > 0 then foundId = n break end
									end
								end
								if foundId == 0 then
									local meshMatch = tostring(handle.MeshId or ""):match("(%d+)")
									if meshMatch then foundId = tonumber(meshMatch) or 0 end
								end
								if foundId > 0 then
									table.insert(accs, { AssetId = foundId, AccessoryType = Enum.AccessoryType.Unknown, Order = #accs + 1 })
								end
							end
						end
					end
		...
		if #accs > 0 then
			makeInvHeader(InvScroll, "Accessories (" .. #accs .. ")", order)
			order = order + 1
			for i, acc in ipairs(accs) do
				local accType = tostring(acc.AccessoryType)
				local capturedAssetId = acc.AssetId
				buildInvCard(
					acc.AssetId,
					"Accessory #" .. tostring(acc.AssetId),
					"Type: " .. accType .. "  |  ID: " .. tostring(acc.AssetId),
					"Accessory  |  Order: " .. tostring(acc.Order or i),
					order,
					function()
						local newAccs = {}
						pcall(function()
							local cur = CurrentHumDes:GetAccessories(true)
							for _, a in ipairs(cur) do
								if a.AssetId ~= capturedAssetId then
									table.insert(newAccs, a)
								end
							end
							CurrentHumDes:SetAccessories(newAccs, true)
						end)
						pcall(function()
							local char = LocalPlayer.Character
							if char then
								for _, obj in ipairs(char:GetChildren()) do
									if obj:IsA("Accessory") then
										...
										if handle then
											...
											if foundId == capturedAssetId or obj.Name == tostring(capturedAssetId) then
												obj:Destroy()
												break
											end
										end
		...
		local bodyProps = {
			{ prop = "Shirt", label = "Shirt", cat = "Clothing" },
			{ prop = "Pants", label = "Pants", cat = "Clothing" },
			{ prop = "Face", label = "Face", cat = "Body" },
			{ prop = "Head", label = "Head", cat = "Body" },
			{ prop = "Torso", label = "Torso", cat = "Body" },
			{ prop = "LeftArm", label = "Left Arm", cat = "Body" },
			{ prop = "RightArm", label = "Right Arm", cat = "Body" },
			{ prop = "LeftLeg", label = "Left Leg", cat = "Body" },
			{ prop = "RightLeg", label = "Right Leg", cat = "Body" },
		}

		local hasBody = false
		for _, p in ipairs(bodyProps) do
			local val = 0
			pcall(function() val = CurrentHumDes[p.prop] end)
			if val and val ~= 0 then hasBody = true end
		end

		if hasBody then
			makeInvHeader(InvScroll, "Clothing & Body Parts", order)
			order = order + 1
			for _, p in ipairs(bodyProps) do
				local val = 0
				pcall(function() val = CurrentHumDes[p.prop] end)
				if val and val ~= 0 then
					local capturedProp = p.prop
					buildInvCard(
						val,
						p.label,
						p.cat .. "  |  ID: " .. tostring(val),
						"Asset: " .. tostring(val),
						order,
						function()
							pcall(function() CurrentHumDes[capturedProp] = 0 end)
							applyProperty(capturedProp, 0)
						end
					)
					order = order + 1
				end
		...
		if order == 0 then
			...
			emptyLbl.Parent = InvScroll
			emptyLbl.Size = UDim2.new(1, 0, 0, 60)
			emptyLbl.BackgroundTransparency = 1
			emptyLbl.Text = "No items equipped.\nCopy a skin first or equip assets from Tools tab."
	...
	RefreshAllBtn.MouseButton1Click:Connect(function()
		refreshInventory()
		setStatus("Inventory refreshed!", true)
	end)
...
table.insert(eventConnections, LocalPlayer.CharacterAdded:Connect(function()
	if AutoReapplyOnRespawn and CurrentHumDes then
		task.delay(1.5, function()
			if CurrentHumDes then
				_G.SalamangueClonerApply()
				setStatus("Skin re-applied after respawn!", true)
			end
		end)
	end
end))
...
print("[Salamangue Cloner Ultimate] Initialized successfully.")
